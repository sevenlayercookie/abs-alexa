...
Pushes `lambda/` to the skill's Alexa-hosted repository and updates the manifest
and interaction model, while preserving the three things that belong to the
skill rather than to this repo:

| field | why it must survive |
|---|---|
| skill name | distinguishes your skills in the console |
| invocation name | what you actually say to Alexa |
| `apis.custom.endpoint` and `regions` | generated by Amazon at skill creation |

It also leaves `config.js` in the hosted repository alone, since Alexa-hosted
has no environment-variable configuration and that file holds the credentials.
