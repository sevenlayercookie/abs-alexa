...
Verified directly: open a session and it reads `200`, sync it and it still reads
`200`, close it and the same id reads `404`.
...
Verified directly: open + sync `timeListened: 0` + close `timeListened: 0` left
the session count unchanged, while open + sync `timeListened: 130` + close
`timeListened: 0` produced a row reading 130 s. So listening time accumulates
across syncs, and the close does not need to carry any of it.
...
Verified directly: a `PATCH` left the session count unchanged.
